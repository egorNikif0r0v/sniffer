...
int main() {
    ...
    pcap_loop(pcap_handle, INFINITY, caught_packet, nullptr);
...
void caught_packet(u_char *user_args, const struct pcap_pkthdr *cap_header, const u_char *packet) {
    ...
    if(pkt_data_len > 0) {
        ...
        forming_logs(pkt_data, pkt_data_len, filename);
    } else{
...
void decode_ethernet(const u_char *header_start) {
    int i;
    auto ethernet_header = (const struct ether_hdr *)header_start;
...
void decode_ip(const u_char *header_start) {
    ...
    ip_addr.s_addr = ip_header->ip_dest_addr;
    printf("Dest: %s )\n", inet_ntoa(ip_addr));
